...
public class PlayerDeckCondition : MonoBehaviour
{
    ...
    public int CurrentClickableMapColumn = 0;
    private bool holdersChecked = false;
...
//THIS IS FOR STANDARD COMBAT
    ...
    {
        Debug.Log("GamePlayGen Start");
        if (holdersChecked == false)
        {
        ...
        //if and elseif are used to spawn the 1st 2 sets of cards at the start of battle
        if (UnitDeckIndex == 0)
        {
    ...
    public void HandChecker(bool spawning)
    {
        ...
        for (int i=0; i<BattleCardHandVectors.Length; i++)
        {
            Collider2D[] hit = Physics2D.OverlapPointAll(BattleCardHandVectors[i], Cards, -Mathf.Infinity, Mathf.Infinity);
            for (int j=0; j<hit.Length; j++)
            {
                if (hit != null)
                {
                    HeldCards[i] = hit[j].gameObject;
                    Debug.Log(HeldCards[i]);
                } else { HeldCards[i] = null; }
            }
            
        }

        /*if (hit != null)   //hit returns the card in that holder
        {
            HeldCards[0] = hit.gameObject;
            Debug.Log(HeldCards[0]);
    ...
    public void SpellCardGen(int cardSpawnLocation)
    {
        ...
        PSpell2.GetComponent<BoxCollider2D>().size = new Vector2(5.7f, 8f);
...
//----------------------------------------------------------------------------------------------------
    ...
    public int ClickableMapColumnGetter()
    {
        return CurrentClickableMapColumn;
    }
    public void ClickableMapColumnIncreaser()
    {
        CurrentClickableMapColumn++;
    }

    public void UnitDeckPositionReset()
    {
        UnitDeckIndex = 0;
    }
    public void SpellDeckPositionReset()
    {
        SpellDeckIndex = 0;
    }
    public void HolderCheckReset()
    {
